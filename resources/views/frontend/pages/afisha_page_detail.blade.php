...
@section('content')
    <section class="pt-16 pb-12 px-6" style="background: var(--dark-surface); border-bottom: 1px solid var(--dark-border);">
        <div class="max-w-4xl mx-auto">
            <a href="{{ route('frontend.afisha') }}" class="inline-flex items-center text-sm mb-6 transition-colors"
                style="color: var(--gold);">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                </svg>
                @if(session()->get('lang') == 'ru') Вся афиша
                @elseif(session()->get('lang') == 'en') All events
                @else Ҳамаи афиша @endif
            </a>
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div class="grid md:grid-cols-3 gap-8">
                    @foreach($related_news->take(3) as $item)
                        <a href="{{ route('frontend.afisha.detail', $item->id) }}" class="related-card block">
                            <div class="h-48 overflow-hidden">
                                @if(!empty($item->image) && $item->image !== 'no-image.jpg')
                                    <img src="{{ asset($item->image) }}" class="w-full h-full object-cover" alt="{{ $item->title_ru }}">
                                @else
